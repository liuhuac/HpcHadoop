...
# set JAVA_HOME dynamically in case versions are different on compute nodes.
echo "" >> $CONFIG_DIR/hadoop-env.sh
echo "# Set JAVA_HOME" >> $CONFIG_DIR/hadoop-env.sh
echo "export JAVA_HOME=$(echo $(dirname $(dirname $(which java))))" >> $CONFIG_DIR/hadoop-env.sh
